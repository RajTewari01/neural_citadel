...
Features :
    ...
    >>> Helps to determine the platform and its venv for proper initialization.
    
NOTE : It for all the base work and no heavy modules or work has been ititialized in this module. 
       This will helps the developer to not perform repetative task.
...
class DetailsChecker:
    ...
    '''
    def __init__(self):
        self.__ROOT__ =  Path(__file__).resolve().absolute().parents[4]
    ...
    def __checkfile(
        ...
        returntype: Optional[Union[str,Path]]
    ) -> Optional[Union[Path,str]]:

        search_path = self.__ROOT__/ 'assets'
        if not filename: raise ValueError('File name cannot be none.')
        if  dir_path:
